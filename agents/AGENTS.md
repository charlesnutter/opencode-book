...
This file is the single source of truth. The repo root carries symlinks so each
tool finds it at its conventional path:

```
agents/AGENTS.md     <- the real file, edit this
AGENTS.md            -> agents/AGENTS.md   (opencode)
CLAUDE.md            -> agents/AGENTS.md   (Claude Code)
```

Agent *definitions* (markdown subagents) are a separate concern from this
instruction file. When you add them, put them in `agents/definitions/` and
symlink `.opencode/agents` and `.claude/agents` to it, so those stay
single-sourced too.
